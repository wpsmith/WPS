...
class Manual extends WPS\Core\Post_Type {
	...
	public function core_acf_fields( $fields ) {
		...
		$content
			->addFile( 'manual', array(
				'label'    => __( 'Field Manual', SITE_MU_TEXT_DOMAIN ),
				'multiple' => true,
			) )
...
//		WPS\printr( $manuals_vans );
		...
		printf( '<a target="_blank" href="%s" class="button accent alignright">%s</a>', get_permalink() . $slug . '/', __( 'Open', SITE_MU_TEXT_DOMAIN ) );
		echo '</div></div>';
		echo '</div></div>';
	}
