...
if ( ! class_exists( 'Van' ) ) {
	class Van extends WPS\Core\Post_Type {
		...
		public function core_acf_fields( $fields ) {
			...
			$specs
				->addText( 'make', array(
					'label' => __( 'Make', SITE_MU_TEXT_DOMAIN ),
				) )
				->addText( 'model', array(
					'label' => __( 'Model', SITE_MU_TEXT_DOMAIN ),
				) )
				->addText( 'ramp-width', array(
					'label' => __( 'Ramp width (inches)', SITE_MU_TEXT_DOMAIN ),
				) )
				->addText( 'ramp-length', array(
					'label' => __( 'Ramp length (inches)', SITE_MU_TEXT_DOMAIN ),
				) )
				->addText( 'ramp-angle', array(
					'label' => __( 'Ramp angle (degrees)', SITE_MU_TEXT_DOMAIN ),
				) )
				->addText( 'door-opening-height', array(
					'label' => __( 'Door opening height (inches)', SITE_MU_TEXT_DOMAIN ),
				) )
				->addText( 'interior-smallest-width', array(
					'label' => __( 'Interior width smallest (inches)', SITE_MU_TEXT_DOMAIN ),
				) )
				->addText( 'interior-depth', array(
					'label' => __( 'Interior depth (inches)', SITE_MU_TEXT_DOMAIN ),
				) )
				->addText( 'ground-clearance', array(
					'label' => __( 'Ground Clearance (inches)', SITE_MU_TEXT_DOMAIN ),
				) )
				->setLocation( 'post_type', '==', $this->post_type );

			$column_choices = array(
				'choices' => array(
					array( '' => __( 'Full Width', SITE_MU_TEXT_DOMAIN ) ),
					array( 'one-half' => __( 'One Half', SITE_MU_TEXT_DOMAIN ) ),
					array( 'one-third' => __( 'One Third', SITE_MU_TEXT_DOMAIN ) ),
					array( 'one-fourth' => __( 'One Fourth', SITE_MU_TEXT_DOMAIN ) ),
					array( 'one-sixth' => __( 'One Sixth', SITE_MU_TEXT_DOMAIN ) ),
				),
			);
			...
			$manuals
				->addText( 'manual_title', array(
					'default'     => __( 'Field Manual', SITE_MU_TEXT_DOMAIN ),
					'placeholder' => __( 'Field Manual', SITE_MU_TEXT_DOMAIN ),
				) )
				->addRelationship( 'manuals_vans', array(
					'label'                   => __( 'Connected Manuals & Vans', SITE_MU_TEXT_DOMAIN ),
			...
			$galleries
				->addText( 'gallery_title', array(
					'default'     => __( 'Gallery', SITE_MU_TEXT_DOMAIN ),
					'placeholder' => __( 'Gallery', SITE_MU_TEXT_DOMAIN ),
				) )
			...
			$videos
				->addText( 'videos_title', array(
					'default'     => __( 'Repair Videos', SITE_MU_TEXT_DOMAIN ),
					'placeholder' => __( 'Repair Videos', SITE_MU_TEXT_DOMAIN ),
				) )
				->addSelect( 'video_columns', $column_choices )
				->setDefaultValue( 'one-third' )
				->addRelationship( 'videos_vans', array(
					'label'                   => __( 'Connected Videos & Vans', SITE_MU_TEXT_DOMAIN ),
					'acf_relationship_create' => 1,
...
//			'genesis-simple-sidebars',
		...
		public function field_manual() {
			...
			if( rcp_is_active() ) {
				...
				$title = $title ? $title : _n( 'Field Manual', 'Field Manuals', count( $manuals_vans ), SITE_MU_TEXT_DOMAIN );

				// Field Slug
				$slug  = _n( 'manual', 'manuals', count( $manuals_vans ), SITE_MU_TEXT_DOMAIN );
				...
				printf( '<a target="_blank" href="%s" data-id="%s" class="button accent alignright">%s</a>', get_permalink() . $slug, $manuals_vans[0], __( 'Open', SITE_MU_TEXT_DOMAIN ) );
				echo '</div></div>';
				echo '</div></div>';
		...
		public function videos() {
			...
			if( rcp_is_active() ) {
				...
				$title = $title ? $title : __( 'Repair Videos', SITE_MU_TEXT_DOMAIN );
				...
				// Do view all button
//			$slug = _n( 'video', 'videos', count( $videos_vans ), SITE_MU_TEXT_DOMAIN );
...
//			echo '<div class="one-third"><div class="wrap">';
//			printf( '<a target="_blank" href="%s" class="button accent alignright">%s</a>', get_permalink() . $slug . '/', __( 'See All', SITE_MU_TEXT_DOMAIN ) );
//			echo '</div></div>';
//			echo '</div></div>';
		...
		public function gallery_title() {
			...
			$title = $title ? $title : __( 'Gallery', SITE_MU_TEXT_DOMAIN );
			printf( '<h3>%s</h3>', $title );
